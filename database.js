...
function buildEventConfigSeedPayload(source = window.config) {
  ...
  const dateISO = String(source?.event?.dateISO || "").trim();
  const safeDate = dateISO ? new Date(dateISO) : null;
  const formattedDate = safeDate && !Number.isNaN(safeDate.getTime())
    ? safeDate.toISOString().slice(0, 10)
    : String(source?.pareja?.fecha || "").trim() || "2026-11-29";
  const formattedTime = safeDate && !Number.isNaN(safeDate.getTime())
  ...
  return {
    ...
    bride: source?.pareja?.novia || "Novia",
    groom: source?.pareja?.novio || "Novio",
    date: formattedDate,
    time: formattedTime,
